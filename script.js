...
/* =========================================================
   SECTION: CHAT SYSTEM (Refined & Fixed)
   ========================================================= */
...
async function startChat(partnerId, listingId = null) {
    ...
    State.currentListingId = listingId; // Captured for loadConversationThread

    navigate('messages');
}

async function loadMessages() {
    if (!State.user) return; // Silent return if not logged in

    // Fetch messages and join with profile data for both sender and receiver
    ...
    // Map conversations to unique partners
        ...
        sortedConversations.forEach(convo => {
            ...
            li.innerHTML = `
                <div class="convo-avatar">
                    ${partnerProfile.avatar_url ? `<img src="${partnerProfile.avatar_url}" />` : (partnerProfile.username || '?').charAt(0).toUpperCase()}
                </div>
                <div class="convo-details">
                    <div class="convo-username">${escHtml(partnerProfile.username)}</div>
                    <div class="convo-preview">${escHtml(previewText.substring(0, 35))}${previewText.length > 35 ? '...' : ''}</div>
                </div>
            `;
    ...
    // Handle initial state or active chat selection
...
async function loadConversationThread(partnerId, listingId = null) {
    ...
    document.getElementById('activeChatHeader').innerHTML = `Chatting with <strong>${escHtml(partnerProfile.username)}</strong>`;
    document.getElementById('chatForm').style.display = 'flex';

    // Handle auto-fill if coming from a specific listing
    if (listingId) {
        ...
        State.currentListingId = null; // Clean up so it doesn't re-fill on refresh
    }
...
function renderMessage(msg, currentUserId) {
    ...
    div.innerHTML = msg.image_url 
        ? `<img src="${escHtml(msg.image_url)}" class="msg-image" onclick="window.open(this.src)">`
        : escHtml(msg.content);
...
function initChat() {
    ...
    .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'messages' }, payload => {
        ...
        if (State.currentPage === 'messages' && relevantToMe) {
            // If it's the current open conversation, render it
            ...
            // Always refresh the list to update previews/order
            loadMessages();
        }
    })
    .subscribe();
