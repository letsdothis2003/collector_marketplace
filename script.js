...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  const currentPage = document.body.getAttribute('data-page') || 'shop';
  
  if (currentPage === 'shop') {
    loadListings();
  } else if (currentPage === 'profile' && State.user) {
    loadProfile();
  } else if (currentPage === 'wishlist' && State.user) {
    loadWishlist();
  } else if (currentPage === 'create' && State.user) {
    initCreatePage();
  } else if (currentPage === 'messages' && State.user) {
    loadMessages();
